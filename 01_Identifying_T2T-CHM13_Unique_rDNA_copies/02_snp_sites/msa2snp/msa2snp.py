        ...
        for line in file_one:
            ...
            if line:  # skip blank lines
                if line.startswith(">"):
                    ...
                    # ADDED BY AISHA -- TO CAPITALIZE ALL CHARACTERS
                    fasta[sequence_name] += line.replace(" ", "").upper()

    return fasta, seqnames


# algnment file, fasta format, first one is the reference
aln_file = sys.argv[1]
...
for k in range(len(refseq_nogap)):
    ...
    if len(alleles) != 1:
        ...
        outlist.append([str(k+1), ref_allele, alt_allele] +
                       seq)  # K+1: starting from 1
...
for i in range(len(outlist) - 1):
    ...
    if int(p1[0]) + 1 == int(p2[0]) and "-" in p1 and "-" in p2 and allpos(p1, "-") == allpos(p2, "-"):
        ...
        if mm:
            ...
            mm[0] = n
            # replace "-----" to "-"
            outlist2.append(["-" if "-" in w else w for w in mm])
            mm = []
        else:
            outlist2.append(p1)
